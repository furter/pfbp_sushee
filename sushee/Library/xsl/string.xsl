...
`/sushee/Library/xsl/string.xsl` is part of Officity.
...
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:doc="http://xsltsl.org/xsl/documentation/1.0" xmlns:str="http://xsltsl.org/string" extension-element-prefixes="doc str">

	<!-- Common string constants and datasets as XSL variables -->
	<!-- lower and upper contain pairs of lower and upper case characters. Below insanely long strings should contain the official lower/uppercase pairs, making this stylesheet working for every language on earth. Hopefully. -->
	<!-- These values are not enough, however. There are some exceptions, dealt with below. -->
	...
	<xsl:template name="replace-all-string">
		...
		<xsl:param name="text"/>
		<xsl:param name="froms"/>
		<xsl:param name="tos"/>
		
		<xsl:variable name="replaced_text">
			<xsl:choose>
				<xsl:when test="contains($froms,',')">
					<xsl:variable name="text_tmp">
						<xsl:call-template name="replace-string">
							<xsl:with-param name="text" select="$text" />
							<xsl:with-param name="from" select="substring-before($froms,',')" />
							<xsl:with-param name="to" select="substring-before($tos,',')" />
						</xsl:call-template>
					</xsl:variable>
					
					<xsl:call-template name="replace-all-string">
						<xsl:with-param name="text" select="$text_tmp" />
						<xsl:with-param name="froms" select="substring-after($froms,',')" />
						<xsl:with-param name="tos" select="substring-after($tos,',')" />
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="replace-string">
						<xsl:with-param name="text" select="$text" />
						<xsl:with-param name="from" select="$froms" />
						<xsl:with-param name="to" select="$tos" />
					</xsl:call-template>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<!-- Camelcase -->	
	<xsl:template name="to-camelcase">
		<xsl:param name="text"/>
	...
	<xsl:template name="substring-after-at">
		<xsl:param name="text"/>
		<xsl:param name="chars"/>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:call-template name="substring-after-at">
					<xsl:with-param name="text" select="substring-after($text, $chars)"/>
					<xsl:with-param name="chars" select="$chars"/>
	...
	<xsl:template name="substring-before-at">
		<xsl:param name="text"/>
		<xsl:param name="chars"/>
		<xsl:param name="position"/>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of select='substring-before($text, $chars)'/>
				<xsl:value-of select='$chars'/>
				<xsl:call-template name="substring-before-at">
					<xsl:with-param name="text" select="substring-after($text, $chars)"/>
					<xsl:with-param name="position" select="$position - 1"/>
					<xsl:with-param name="chars" select="$chars"/>
				</xsl:call-template>
	...
	<xsl:template name="backward">
		...
		<xsl:variable name="mirror">
			<xsl:call-template name="build-mirror">
				<xsl:with-param name="text" select="$text"/>
	...
	<xsl:template name="build-mirror">
		<xsl:param name="text"/>
		<xsl:param name="position"/>
		<xsl:choose>
			<xsl:when test="$position &gt; 0">
	...
	<xsl:template name="character-first">
		<xsl:param name="text"/>
		<xsl:param name="chars"/>
		...
		<xsl:choose>
			...
			<xsl:when test="contains($text, substring($chars, 1, 1))">
				<xsl:variable name="next-character">
					<xsl:call-template name="character-first">
						<xsl:with-param name="text" select="$text"/>
						<xsl:with-param name="chars" select="substring($chars, 2)"/>
					</xsl:call-template>
				</xsl:variable>
	...
	<xsl:template name="string-match">
		<xsl:param name="text"/>
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					...
					<xsl:when test="$special = '*'">
						<xsl:call-template name='match-postfix'>
					...
					<xsl:when test="$special = '?'">
						<xsl:call-template name="string-match">
	...
	<xsl:template name="match-postfix">
		<xsl:param name="text"/>
		<xsl:param name="pattern"/>
