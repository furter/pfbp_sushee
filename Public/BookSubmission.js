...
//	alert("step:"+step+" current_step:"+current_step+" step_filling:"+step_filling);
	if (step <= step_filling) {
		...
		if ($('#specific_mask>div:eq('+index+')').hasClass('BookSubmissionSummary')) {
			...
			//bookRemarkMore = bookFinalRemarks.replace( bookRemarksMerged );
//			alert('merged: '+bookRemarksMerged+' //// ajouté: '+bookRemarkMore);
			summarizeSubmission();
			...
			// if ( bookFinalRemarks == '') {
				...
				bookRemarksMerged = c_remarks;
				c_remarks = c_remarks + bookRemarkMore;
			/*} else {
				...
				for (var i=0; i < all_fields.length; i++) {
					...
					if (reponse[field]) {
						...
						if (reponse[field] != '' && reponse[field] != 'und') {
							$(form_target+' #'+field).attr('disabled', 'disabled').addClass('disabled');
						} else {
							$(form_target+' #'+field).attr('disabled', 'disabled').removeClass('disabled');
						}
					};
				};
